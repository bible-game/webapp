...
const verseColor = (verse: any) => {
    ...
    if (verse === 0) return "bg-[#C9A84C] opacity-85";
    return "bg-[#5BB88A] opacity-85";
};
...
const Heatmap = (props: Readonly<Props>) => {
    ...
    return (
        <section className="sm:w-[48rem] w-[80vw] pb-12 mt-6">
            <div className="border border-white/[0.15] bg-gradient-to-br from-[#050730]/80 to-[#040127]/80 backdrop-blur-md rounded-lg p-4 sm:p-5 shadow-lg">
                <h3 className="text-sm font-medium text-white/70 mb-4">Bible Completion</h3>

                {!hasAnyCompletion && (
                    <div className="text-center py-6 mb-4 rounded-md bg-white/[0.02] border border-dashed border-white/10">
                        <p className="text-sm text-white/30 font-light">No progress yet</p>
                        <p className="text-xs text-white/20 font-light mt-1">Play games and read passages to track your completion here</p>
                    </div>
                )}
                                        ...
                                        <div key={book.key}>
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 h-1.5 rounded-full bg-white/[0.15] overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full transition-all duration-500 ${pct === 0 ? '' : pct < 1 ? 'bg-[#C9A84C]' : 'bg-[#5BB88A]'}`}
