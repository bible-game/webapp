...
const Results = (props: any) => {
    const [today] = React.useState(moment(new CalendarDate(parseInt(props.date.split('-')[0]), parseInt(props.date.split('-')[1]), parseInt(props.date.split('-')[2]))).format('Do MMMM YYYY'));
    let stars = 0;
    let message = '';
    let result = '';
...
${today}`;
    ...
    return <Button className="bg-transparent text-white text-xs flex gap-2 items-center cursor-pointer relative z-10 h-8" onClick={() => {navigator.clipboard.writeText(result)}}>
